...
stockDataPathList = ['csvData_basic//', 'csvData_derivative//']#['csvData_basic//']#['E://DeepQuant//raw_data//csvData_updated//']#['csvData_basic//', 'csvData_moneyflow//']
...
def HandleEachStockData(fileName, removeSubnew):
	...
	for stockDataPath in stockDataPathList:
		oneFileData = pd.read_csv(os.path.join(stockDataPath, fileName), parse_dates=[1], index_col=[1]).drop('Unnamed: 0', axis = 1)
...
def generateMarketData():
	...
	dataNameList = ['TRADE_DT', 
					'AVGDELTA', 'AVGTURNOVER', 'AVGFREETURNOVER', 'UPSTOCKRATIO_0', 'UPSTOCKRATIO_5', 'DOWNSTOCKRATIO_5', 
...
if __name__ == '__main__':
	...
	for fileName in os.listdir(stockDataPathList[0]):
		...
		HandleEachStockData(fileName, removeSubnew)
