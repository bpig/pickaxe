...
def load_data(states, data_length, interval):
    for state in states:
        current_dir = os.path.dirname(os.path.realpath(__file__))
        DATA_DIR = "data/%s_pydata_L%d_T%d/" % (
            state, data_length, interval)
        ...
        if state == 'test':
            ...
            globals()[state + '_saledate'] = np.load(DATA_DIR +
                                                     state + 'SaleDate.npy')
            globals()[state + '_buydate'] = np.load(DATA_DIR +
                                                    state + 'BuyDate.npy')
        ...
        if state == 'train':    
            globals()[state + '_turnover'] = np.load(DATA_DIR +
                                                     state + 'Turnover.npy')
            globals()[state + '_vr'] = np.load(DATA_DIR +
                                                     state + 'VR.npy')
...
def train(save_path, interval, data_length, nb_channel, delta_scale=20., lr=0.01, gamma=0.1, n_epochs=[5, 6, 6], weight_decay=0.001, batch_size=128):
    current_dir = os.path.dirname(os.path.realpath(__file__))
    states = ['train', 'test']
...
def compute_profits(maxNp, maxNi, sumP, sumRealProfit, date_num, cost, interval, test_start_day, test_end_day, threshold, delta_scale, index_dict_first, index_dict_last, name, weight=False):
    logger = logging.getLogger(name)
    logger.info('--------------------------------------')
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        if os.path.exists(os.path.join(current_dir, 'L%d/' % data_length)) == False:
            os.mkdir(os.path.join(current_dir, 'L%d/' % data_length))
	if os.path.exists(os.path.join(current_dir, 'L%d/' % data_length + '%s/'%model_name)) == False:
